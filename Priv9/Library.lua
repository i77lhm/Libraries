...
-- Library functions 
    ...
    -- Library element functions
        function library:window(properties)
            local cfg = {
                name = properties.name or properties.Name or "fagghaxz",
        ...
        local watermark = library:watermark({name = "fagghaxz - 100 fps - 100 ping"})
        local fps = 0
        ...
        run.RenderStepped:Connect(function()
            ...
            if tick() - watermark_delay > 1 then 
                ...
                watermark.update_text(string.format("fagghaxz - fps: %s - ping: %s", fps, ping))
                fps = 0
            end
        end)
